...
@onready var ray_container = $"../../Camera3D/RayContainerPistol"

@export var can_fire = true

@export var active_weapon = true

@export var max_rounds_in_gun = 6
@export var ammo_in_gun = 6
@export var ammo_in_inv = 32

@export_category("Weapon mechanics")
@export var pistol_damage = 30
@export var pistol_spread = 1
...
func _ready():
	...
	for r in ray_container.get_children():
			r.target_position.x = randi_range(pistol_spread, -pistol_spread)
			r.target_position.y = randi_range(pistol_spread, -pistol_spread)
	...
	for r in ray_container.get_children():
			...
			if r.is_colliding():
				...
				if r.get_collider().is_in_group("Enemy"):
					r.get_collider().health -= pistol_damage
